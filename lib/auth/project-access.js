...
async function requestResourcesBelongToProject(request, params, projectId, bodyResourceModelOverrides = {}) {
  ...
  if (body && typeof body === 'object' && !Array.isArray(body)) {
    for (const [field, model] of Object.entries(BODY_RESOURCE_MODELS)) {
      let resolvedModel = Object.prototype.hasOwnProperty.call(bodyResourceModelOverrides, field)
        ? bodyResourceModelOverrides[field]
        : model;
...
export function withProjectAccess(handler, { bodyResourceModelOverrides = {} } = {}) {
  return async function projectAccessHandler(request, context = {}) {
    try {
      ...
      if (!(await requestResourcesBelongToProject(request, params, access.project.id, bodyResourceModelOverrides))) {
