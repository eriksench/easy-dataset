...
function getHeaders(accessToken, headers = {}) {
  const token = process.env.KNOW_HUB_INTEGRATION_TOKEN;
  if (!token) throw new Error('KNOW_HUB_INTEGRATION_TOKEN is not configured');
  if (!accessToken) throw new Error('Know-Hub user access token is missing');
  return { ...headers, 'X-Integration-Token': token, Authorization: `Bearer ${accessToken}` };
}
...
export async function searchDepartmentFiles({ fileName = '', secretLevel = '', pageNo = 1, pageSize = 10, accessToken }) {
...
export async function downloadDepartmentFile(fileIdentifier, accessToken) {
...
export async function uploadDepartmentFile(file, metadata, accessToken) {
  const formData = new FormData();
  formData.append('file', file, metadata.fileName || file.name);
  formData.append('metadata', new Blob([JSON.stringify(metadata)], { type: 'application/json' }));

  const response = await fetchWithTimeout(
    `${getBaseUrl()}/departFiles/upload`,
    {
      method: 'POST',
      headers: getHeaders(accessToken),
      body: formData
  ...
  if (!response.ok) {
    throw new Error(`Know Hub upload failed (${response.status}): ${await getUpstreamError(response)}`);
  }

  const data = await response.json();
  if (data.code !== 200 || !data.result?.fileIdentifier) {
    throw new Error(data.message || 'Know Hub upload failed');
  }
  return data.result;
}
